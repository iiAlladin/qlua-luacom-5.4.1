                 ...
                 )
{
  ...
  lock_count              = 0;
  conn_point              = NULL;
  conn_cookie = 0; // <--- Инициализация conn_cookie
...
#ifdef VERBOSE
  {
    ...
    if(lua_getstack(L, 1, &ar))
    {
      ...
      snprintf(msg, sizeof(msg), "%.4d:created:LUAINFO=(%s,%d,%s,%s)", ID, ar.short_src, ar.currentline, ar.name, ar.what);
    }
    ...
    {
        snprintf(msg, sizeof(msg), "%.4d:created:LUAINFO=(not available)", ID);
    }
...
bool tLuaCOM::getDISPID(const char* name, DISPID* dispid)
{
    ...
    std::wstring wname;
    wname.resize(n); // включает нулевой терминатор

    if (MultiByteToWideChar(CP_ACP, 0, name, -1, &wname[0], n) <= 0)
        return false;

    // COM API принимает массив указателей на строки
    LPOLESTR pname = &wname[0];

    HRESULT hr = pdisp->GetIDsOfNames(
        IID_NULL,
        &pname,     // массив из одного LPOLESTR
        1,
...
void tLuaCOM::getHelpInfo(char **ppHelpFile, DWORD *pHelpContext)
{
  ...
  size_t str_size = SysStringLen(helpfile);
