    ...
    ULONG cReq, IConnectionPoint** paConnPts, ULONG* pcEnumerated)
{
  ...
  if (pcEnumerated)
    //*pcEnumerated = i;
    *pcEnumerated = static_cast<ULONG>(i);
...
{
    // Используем ULONG для всех вычислений
    ULONG next = static_cast<ULONG>(mNextIndex) + cSkip;

    // Проверяем переполнение при сложении
    if (next < static_cast<ULONG>(mNextIndex)) // Произошло переполнение
        return S_FALSE;

    if (next >= static_cast<ULONG>(mPoints.size()))
        return S_FALSE;

    mNextIndex = static_cast<size_t>(next);
    return S_OK;
}
...
tLuaCOMConnPoint::tLuaCOMConnPoint(lua_State *p_L, IUnknown* pHost) :
  ...
  mConnections(), mCookies(),
    mIIDSink(IID_NULL), // ← ИНИЦИАЛИЗАЦИЯ GUID 
    mpTypeinfo(nullptr) // ← ИНИЦИАЛИЗАЦИЯ УКАЗАТЕЛЯ
{
...
{
    // Преобразуем mNextIndex в size_t для вычислений
    size_t current = static_cast<size_t>(mNextIndex);
    size_t next = current + static_cast<size_t>(cSkip);

    if (next >= mConnections.size())
        return S_FALSE;

    // Проверяем, что результат помещается в ULONG
    if (next > static_cast<size_t>(std::numeric_limits<ULONG>::max()))
        return S_FALSE;

    mNextIndex = static_cast<ULONG>(next);
    return S_OK;
}
