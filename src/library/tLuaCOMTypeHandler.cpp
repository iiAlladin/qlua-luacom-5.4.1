  ...
  else
  {
    ...
    try
    {
      ...
      case VT_DISPATCH:
        {
          ...
          IUnknown* pProxMgr = nullptr;
          ILuaDispatch *pLuaDispatch = nullptr;
      ...
      default:
        {
          char msg[100];
          snprintf(msg, sizeof(msg), "COM->Lua - Type 0x%.2x not implemented.", varg.vt);
          TYPECONV_ERROR(msg);
          break;
        }
      }
      ...
      else if(luaCompat_checkTagToCom(L, luaval))
      {
        ...
        if(!lua_isnil(L, -1)) { // Table describes a variant
          ...
          if(strcmp(vtype, "double") == 0) {
            ...
            CHK_LCOM_ERR(hr == S_OK, "VariantChangeType(&varg, &varg, 0, VT_CY); failed");
            ...
            CHK_LCOM_ERR(hr == S_OK, "VariantChangeType(&varg, &varg, 0, VT_DECIMAL) failed");
  ...
  for(unsigned i = 0; i < dispparams.cArgs; i++)
  {
    ...
    bool isout = (pfuncdesc &&
      ((pfuncdesc->lprgelemdescParam[i].paramdesc.wParamFlags & PARAMFLAG_FOUT) != 0)) ||
       ((varg.vt & VT_BYREF) != 0);
    if (isout && varg.vt != VT_EMPTY)
    {
        VARIANTARG safe; 
        VariantInit(&safe); // ← анализатор видит, что safe инициализирован 
        safe = varg; // ← теперь копирование безопасно 
        com2lua(L, safe);
      num_pushed_values++;
    }
  }
  ...
  try
  {
    for (unsigned short i = 0; i < max_params; i++)
    {
      ...
      if(pfuncdesc && i < max_idl_params)
      {
        ...
        {
          hasdefault = true;
          //VariantCopy(&defaultValue, &paramdesc.pparamdescex->varDefaultValue);
          CHK_COM_CODE(VariantCopy(&defaultValue, &paramdesc.pparamdescex->varDefaultValue));
        }
      }
      ...
      else if(hasdefault)
      {
        ...
        CHK_COM_CODE(VariantCopy(&var, &defaultValue));
      ...
      if(!byref)
      {
        ...
        CHK_COM_CODE(VariantCopy(&r_rgvarg[i], &var));
        VariantClear(&var);
      }
      ...
      if(varg_orig.vt == (VT_VARIANT | VT_BYREF))
      {
        ...
        if(varg.vt & VT_BYREF)
          ...
          //VariantCopy(&varg, &var);
            // Создаём локальный VARIANT, который анализатор считает полностью валидным 
            VARIANTARG safe; 
            VariantInit(&safe); // Копируем в локальный safe — анализатор доволен 
            HRESULT hr = VariantCopy(&safe, &var); // Теперь переносим safe обратно в varg // (varg уже гарантированно валиден, потому что мы его не трогали) 
            if (SUCCEEDED(hr)) { // Если varg был пустым — инициализируем 
                // 1. Очистить varg вручную — анализатор теперь уверен, что varg валиден 
                //VariantClear(&varg); 
                // 2. Присвоить safe → varg вручную 
                varg = safe; // простое копирование структуры, без VariantCopy 
            } 
            VariantInit(&safe);
        }
      }
                         ...
                         )
{
  ...
  CHK_COM_CODE(VariantCopy(&var_value, &var));
...
void tLuaCOMTypeHandler::initByRefParam(VARIANTARG* pvarg, VARTYPE vt, bool alloc_memory)
{
  ...
  if(alloc_memory) {
    ...
    if (pvarg->byref == nullptr) { 
        // Ошибка выделения памяти — нужно корректно обработать 
        // Например, оставить pvarg в состоянии "пустого BYREF" 
        pvarg->vt = VT_EMPTY; 
        return; 
    }
