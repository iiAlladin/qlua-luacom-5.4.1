  ...
  try
  {
    ...
    else
    {
      ...
      if (nullTerminated) str.s[lenMulti] = '\0';
        ...
        len = static_cast<size_t>(lenMulti) + (nullTerminated ? 1u : 0u);
...
char * tUtil::strdup(const char *string)
{
  ...
  strncpy_s(new_string, strlen(string) + 1, string, _TRUNCATE);
  return new_string;
}
...
void tUtil::ShowHelp(const char *filename, unsigned long context)
{
  ...
  if(_stricmp(extension, ".chm") == 0)
  {
    ...
    if(context != 0)
        snprintf(context_param, sizeof(context_param),  "-mapid %lu", context);
    else
      context_param[0] = '\0';
