...
static constexpr const char* PUT_PREFIX = "set";
...
// --- BEGIN: additions for registration/expose policy helpers ---

// #include <windows.h> уже подключается через ole2.h, оставлено для явности

// Возвращает true, если процесс выполняется с повышенными правами
...
// Проверяет, разрешена ли регистрация/экспонирование.
// provided_token может быть NULL — тогда сравнение по токену не выполняется.
static bool check_registration_allowed(lua_State* L, const char* provided_token)
{
    // 1) Проверка конфигурационного флага: module.config.allow_registration
    luaCompat_moduleGet(L, MODULENAME, "allow_registration");
    bool allow_flag = lua_toboolean(L, -1) != 0;
    lua_pop(L, 1);
    if (allow_flag)
        return true;

    // 2) Если процесс elevated — разрешаем
    if (IsProcessElevated())
        return true;

    // 3) Сравнение секретного токена (module.config.registration_token)
    if (provided_token && provided_token[0] != '\0')
    {
        luaCompat_moduleGet(L, MODULENAME, "registration_token");
        const char* expected = lua_tostring(L, -1);
        lua_pop(L, 1);
        if (expected && strcmp(expected, provided_token) == 0)
            return true;
    }
...
/**
  ...
  Returns
    ...
    tabela fornecida ou nil se nao for possнvel estabelecer a
    conexao
...
{
    size_t len = 0;
    const char* progIdA = luaL_checklstring(L, 1, &len);

    // ANSI → UTF‑16
    int n = MultiByteToWideChar(CP_ACP, 0, progIdA, -1, NULL, 0);
    if (n <= 0)
    {
        lua_pushnil(L);
        return 1;
    ...
    std::wstring progIdW(n, L'\0');
    if (MultiByteToWideChar(CP_ACP, 0, progIdA, -1, &progIdW[0], n) <= 0)
    {
        lua_pushnil(L);
        return 1;
    }
    ...
    // CLSID → wide string
    LPOLESTR clsidW = nullptr;
    hr = StringFromCLSID(clsid, &clsidW);
    if (FAILED(hr) || !clsidW)
    {
        lua_pushnil(L);
        return 1;
    ...
    // wide → ANSI
    std::string clsidA;
    int n2 = WideCharToMultiByte(CP_ACP, 0, clsidW, -1, NULL, 0, NULL, NULL);
    if (n2 > 0)
    {
        clsidA.resize(n2);
        if (WideCharToMultiByte(CP_ACP, 0, clsidW, -1, &clsidA[0], n2, NULL, NULL) <= 0)
            clsidA.clear();
    }
...
{
    size_t len = 0;
    const char* clsidA = luaL_checklstring(L, 1, &len);

    // ANSI → UTF‑16
    int n = MultiByteToWideChar(CP_ACP, 0, clsidA, -1, NULL, 0);
    if (n <= 0)
    {
        lua_pushnil(L);
        return 1;
    ...
    std::wstring clsidW(n, L'\0');
    if (MultiByteToWideChar(CP_ACP, 0, clsidA, -1, &clsidW[0], n) <= 0)
    {
        lua_pushnil(L);
        return 1;
    }
    ...
    // wide → CLSID
    CLSID clsid = IID_NULL;
    if (FAILED(CLSIDFromString(clsidW.c_str(), &clsid)))
    {
        lua_pushnil(L);
        return 1;
    }
    ...
    HRESULT hr = ProgIDFromCLSID(clsid, &progIdW);
    if (FAILED(hr) || !progIdW)
    {
        lua_pushnil(L);
        return 1;
    }
    ...
    // wide → ANSI
    std::string progIdA;
    int n2 = WideCharToMultiByte(CP_ACP, 0, progIdW, -1, NULL, 0, NULL, NULL);
    if (n2 > 0)
    {
        progIdA.resize(n2);
        if (WideCharToMultiByte(CP_ACP, 0, progIdW, -1, &progIdA[0], n2, NULL, NULL) <= 0)
            progIdA.clear();
    }
    ...
    lua_pushstring(L, progIdA.c_str());
    return 1;
}
...
// --- BEGIN: modified ExposeObject ---
// Оригинальная функция luacom_ExposeObject, добавлена проверка прав.
//  Дополнительно допускается необязательный второй аргумент: token (string)
static int luacom_ExposeObject(lua_State* L)
{
    // check parameters
    tLuaCOM* luacom = (tLuaCOM*)LuaBeans::check_tag(L, 1);

    // optional token argument (string) at index 2
    const char* token = NULL;
    if (lua_gettop(L) >= 2 && lua_isstring(L, 2))
        token = lua_tostring(L, 2);

    // policy check
    if (!check_registration_allowed(L, token)) {
        luacom_APIerror(L, "Operation denied: exposing COM objects requires explicit permission (allow_registration), elevated process or valid registration_token.");
        return 0;
    }
...
//
//        {
//            const char* tp = typelib_path;
//            int n = MultiByteToWideChar(CP_ACP, 0, tp, -1, wcTypelib_path, bufsize);
//            CHK_LCOM_ERR(n > 0, "Failed to convert TypeLib path to wide string.");
//        }
//
//        CHK_COM_CODE(LoadTypeLibEx(wcTypelib_path, REGKIND_REGISTER, &typelib));
//
//        // --- Get type library version and LIBID ---
//        char version[30];
//        char libId[bufsize];
//
//        {
...
//
//            snprintf(version, sizeof(version), "%hu.%hu", plibattr->wMajorVerNum, plibattr->wMinorVerNum);
//
//            LPOLESTR wcLibId = NULL;
//            CHK_COM_CODE(StringFromCLSID(plibattr->guid, &wcLibId));
//
//            int n = WideCharToMultiByte(CP_ACP, 0, wcLibId, -1,
//                libId, bufsize, NULL, NULL);
//            CoTaskMemFree(wcLibId);
//            CHK_LCOM_ERR(n > 0, "Failed to convert LIBID to ANSI.");
//
//            typelib->ReleaseTLibAttr(plibattr);
//        }
//
//        // --- Get CoClass TypeInfo and CLSID ---
//        tCOMPtr<ITypeInfo> coclassinfo;
...
//
//            int n = WideCharToMultiByte(CP_ACP, 0, wcClsid, -1,
//                clsid, bufsize, NULL, NULL);
//            CoTaskMemFree(wcClsid);
//            coclassinfo->ReleaseTypeAttr(ptypeattr);
//            CHK_LCOM_ERR(n > 0, "Failed to convert CLSID to ANSI.");
//        }
//
//        // --- Build module path ---
//        char ModulePath[bufsize] = { 0 };
//        GetModuleFileNameA(NULL, ModulePath, sizeof(ModulePath));
//
//        std::string modulePath = ModulePath; 
//        if (scriptFile) { modulePath += " "; modulePath += scriptFile; } 
//        if (arguments) { modulePath += " "; modulePath += arguments; }
//        
//        std::string clsidKey = std::string("CLSID\\") + clsid;
//
//        tCOMUtil::SetRegKeyValue(ProgID, NULL, ComponentName);
//        tCOMUtil::SetRegKeyValue(ProgID, "CLSID", clsid);
//
//        tCOMUtil::SetRegKeyValue(VersionIndependentProgID, NULL, ComponentName);
//        tCOMUtil::SetRegKeyValue(VersionIndependentProgID, "CurVer", ProgID);
//        tCOMUtil::SetRegKeyValue(VersionIndependentProgID, "CLSID", clsid);
//
//        tCOMUtil::SetRegKeyValue(clsidKey.c_str(), NULL, ComponentName);
//        tCOMUtil::SetRegKeyValue(clsidKey.c_str(), "ProgID", ProgID);
//        tCOMUtil::SetRegKeyValue(clsidKey.c_str(), "VersionIndependentProgID", VersionIndependentProgID);
//        tCOMUtil::SetRegKeyValue(clsidKey.c_str(), "LocalServer32", modulePath.c_str());
//
//        if (scriptFile)
//        {
//            tCOMUtil::SetRegKeyValue(clsidKey.c_str(), "InprocServer32", LUACOM_DLL);
//            tCOMUtil::SetRegKeyValue(clsidKey.c_str(), "ScriptFile", scriptFile);
//        }
//
//        if (control)
//        {
//            tCOMUtil::SetRegKeyValue(clsidKey.c_str(),
...
//        }
//
//        tCOMUtil::SetRegKeyValue(clsidKey.c_str(), "Version", version);
//        tCOMUtil::SetRegKeyValue(clsidKey.c_str(), "TypeLib", libId);
//        tCOMUtil::SetRegKeyValue(clsidKey.c_str(), "Programmable", NULL);
//    }
//    catch (class tLuaCOMException& e)
//    {
//        luacom_APIerror(L, e.getMessage());
...
// --- BEGIN: modified RegisterObject ---
// Функция RegisterObject теперь требует разрешения (см. check_registration_allowed).
// Поддерживает опциональный второй аргумент: token (string).
// local luacom = require("luacom")
//--вариант 1: процесс elevated->работает автоматически
//-- вариант 2 : явно разрешить
//luacom.config.allow_registration = true
//
//--вариант 3 : использовать токен
//luacom.config.registration_token = "my-secret"
//--затем при вызове из скрипта :
//luacom.RegisterObject(reg_info, "my-secret")
static int luacom_RegisterObject(lua_State* L)
{
    if (lua_type(L, 1) != LUA_TTABLE && lua_type(L, 1) != LUA_TUSERDATA)
    {
    ...
    // policy check
    if (!check_registration_allowed(L, token)) {
        luacom_APIerror(L, "Operation denied: registering COM objects requires explicit permission (allow_registration), elevated process or valid registration_token.");
        return 0;
    }
    ...
    try
    {
        ...
        if (scriptFile)
        {
            // безопасно добавляем с проверкой длины
            size_t cur = strlen(ModulePath);
            if (cur + 1 + strlen(scriptFile) + 1 < sizeof(ModulePath)) {
                strcat(ModulePath, " ");
        ...
        {
            size_t cur = strlen(ModulePath);
            if (cur + 1 + strlen(arguments) + 1 < sizeof(ModulePath)) {
                strcat(ModulePath, " ");
        ...
        // --- Registry keys ---
        char CLSIDKey[bufsize];
        strncpy_s(CLSIDKey, sizeof(CLSIDKey), "CLSID\\", _TRUNCATE);
        strncat_s(CLSIDKey, sizeof(CLSIDKey), clsid, _TRUNCATE);
        ...
        tCOMUtil::SetRegKeyValue(CLSIDKey, NULL, ComponentName);
        tCOMUtil::SetRegKeyValue(CLSIDKey, "ProgID", ProgID);
        tCOMUtil::SetRegKeyValue(CLSIDKey, "VersionIndependentProgID", VersionIndependentProgID);
        tCOMUtil::SetRegKeyValue(CLSIDKey, "LocalServer32", ModulePath);

        if (scriptFile)
        {
            tCOMUtil::SetRegKeyValue(CLSIDKey, "InprocServer32", LUACOM_DLL);
            tCOMUtil::SetRegKeyValue(CLSIDKey, "ScriptFile", scriptFile);
        }

        if (control)
        {
            // implemented categories...
            tCOMUtil::SetRegKeyValue(CLSIDKey,
                "Implemented Categories\\{0DE86A53-2BAA-11CF-A229-00AA003D7352}",
                NULL);
            // ... остальные как было
        }

        tCOMUtil::SetRegKeyValue(CLSIDKey, "Version", version);
        tCOMUtil::SetRegKeyValue(CLSIDKey, "TypeLib", libId);
        tCOMUtil::SetRegKeyValue(CLSIDKey, "Programmable", NULL);
    }
    catch (class tLuaCOMException& e)
    {
        luacom_APIerror(L, e.getMessage());
...
static int luacom_UnRegisterObject(lua_State* L)
{
    ...
    try
    {
        ...
        // --- Convert TypeLib path to UTF‑16 ---
        wchar_t wcTypelib_path[bufsize];
        {
            const char* tp = typelib_path;
            int n = MultiByteToWideChar(CP_ACP, 0, tp, -1,
                wcTypelib_path, bufsize);
            CHK_LCOM_ERR(n > 0, "Failed to convert TypeLib path to UTF16.");
        }
        ...
        // --- Unregister typelib ---
        {
            ...
            HRESULT hrUnreg = UnRegisterTypeLib(
                plibattr->guid,
        ...
        tCOMUtil::DelRegKey(ProgID, NULL);
        tCOMUtil::DelRegKey(VersionIndependentProgID, NULL);
        tCOMUtil::DelRegKey(clsidKey.c_str(), NULL);
    }
...
*/
static int luacom_CreateLuaCOM(lua_State* L)
{
...
static int luacom_StartMessageLoop(lua_State *L)
{
  MSG msg;
  if(lua_gettop(L) > 0) {
    tStringBuffer err;
...
*/
static int luacom_LuaDetectAutomation(lua_State* L)
{
  lua_getglobal(L,"arg");
  if(lua_istable(L,-1))
  {
...
*/
static int luacom_ImportIUnknown(lua_State* L)
{
...
*/
static int luacom_GetCurrentDirectory(lua_State* L)
{
  char buffer[1025];
...
static int luacom_RoundTrip(lua_State* L) {
    VARIANTARG v;
    VariantInit(&v); // Инициализация VARIANT перед использованием
    ...
    VariantClear(&v); // Очистка VARIANT после использования
...
/**
 Получает указатель void*, который должен быть
 указатель на реализацию
 интерфейс IDispatch. И " создан объект
 Его реализация " определяется этим
 интерфейс. Объект, созданный и " помещенный в
 лунный стек
*/
 ...
 */
extern "C" LUACOM_API void luacom_open(lua_State *L)
{
  if(L == NULL)
    return;
...
// Используйте для Lua 5.4:
  luaL_newlib(L, functions_tb);     // pushes module table on stack

  // prepares to store configuration table in library table
  lua_pushstring(L, CONFIGTABLE_NAME);

  // creates and registers configuration table  (pushes config table)
  luaCompat_moduleCreate(L, MODULENAME);

  // adds a reference of the configuration table to the library one
  // module[CONFIGTABLE_NAME] = config_table
  lua_settable(L, -3);
  //lua_pop(L, 1);
  // register module as global: _G[LIBNAME] = module_table
  lua_setglobal(L, LIBNAME);
  // (module table removed from stack by lua_setglobal)
