...
public:
    ...
    HRESULT _stdcall Next(ULONG celt, OLEVERB* rgelt, ULONG* pceltFetched)
    {
        ...
        for(i = 0; i < celt && mVerbsNext + i < mVerbsSize; ++i)
        {
            ...
            if (mVerbs[verbsi].lpszVerbName != nullptr)
            {
                size_t len = static_cast<size_t>(lstrlenW(mVerbs[verbsi].lpszVerbName));
                size_t bytes = (len + 1) * sizeof(wchar_t);

                rgelt[i].lpszVerbName = static_cast<wchar_t*>(CoTaskMemAlloc(bytes));

                //rgelt[i].lpszVerbName = static_cast<wchar_t*>(
                //    CoTaskMemAlloc((lstrlenW(mVerbs[verbsi].lpszVerbName) + 1) * sizeof(wchar_t)));
                if (rgelt[i].lpszVerbName != nullptr)
                {
                    lstrcpyW(rgelt[i].lpszVerbName, mVerbs[verbsi].lpszVerbName);
                }
                else
                {
                    // Allocation failed, set to nullptr  //возможна €вна€ обработка ошибки Ч оставл€ем nullptr
                    rgelt[i].lpszVerbName = nullptr;
                }
            }
            else
            {
